...
def main():
    ...
    for phase in phases:
        tools.random_seed(0)
        solver = BuckshotSolver(dim=lat_dim, npts=8)
        solver.SetMapper(Pool().map)
        subsolver = NelderMeadSimplexSolver(lat_dim)
        ...
        # find the minimum
        solver.Solve(LatticeParameterCostFunction(detectors, [phase], lat_dim), VTR())
        print(
    ...
    for round in optim_rounds:
        ...
        if round["type"] == "Phase Fraction":
            solver.Solve(PhaseFractionCostFunction(detectors,phases,ndim,ParticleSize=last_particle_size,
                                                   lattice_parameters=lattice_parameters), VTR())
        ...
        if round["type"] == "Particle Size":
            solver.Solve(ParticleSizeCostFunction(detectors,phases,ndim,
                                                  phase_fractions = last_phase_fraction,lattice_parameters=lattice_parameters), VTR())
